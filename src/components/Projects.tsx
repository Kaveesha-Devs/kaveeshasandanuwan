...
const projects = [
  {
    ...
    type: 'Web Apps',
    description: 'A professional website for Yesho Intelligence (Pvt) Ltd, showcasing their services, portfolio, and contact information to enhance their online presence and attract potential clients.',
  ...
  },
  {
    ...
    type: 'Web Apps',
    description: 'A web application that allows users to book movie tickets online, view showtimes, and manage their accounts, providing a seamless and convenient movie ticket booking experience.',
    tags: ['#nextjs', '#tailwind', '#express', '#mongodb', '#stripe', '#clerk', '#inngest'],
    links: { live: 'https://seatify-client.vercel.app/', github: null },
    previewBg: 'from-red-900 to-gray-900',
    previewText: 'Seatify',
  ...
  },
  {
    ...
    type: 'Mobile Apps',
    description: 'Online Lawyer Connect System with exclusive features including online lawyer booking, lawyer rating, and live chat between lawyers and users.',
  ...
  },
  {
    ...
    type: 'Web Apps',
    description: 'Web-based musical instrument shop with CRUD operations, shopping cart, and comprehensive inventory management system.',
    tags: ['#html', '#css', '#php', '#mysql'],
    links: { live: null, github: 'https://github.com/Kaveesha-Devs/melody_master' },
    previewBg: 'from-purple-900 to-pink-900',
  ...
  },
  {
    ...
    title: 'AI Chatbot',
    type: 'AI/ML',
    description: 'A smart chatbot built using Python Flask and OpenAI API with a clean HTML interface for natural language interaction.',
    tags: ['#python', '#flask', '#openai', '#html'],
    links: { live: null, github: 'https://github.com/Kaveesha-Devs/chatbot-project' },
    previewBg: 'from-green-900 to-teal-900',
...
export default function Projects() {
  ...
  const filtered = projects.filter(p => {
    const matchFilter = activeFilter === 'All Projects' || p.type === activeFilter
  ...
  return (
    <section id="projects" ref={ref} className="relative min-h-screen py-24 overflow-hidden" style={{ background: '#050A12' }}>
      ...
      <div className="relative z-10 max-w-6xl px-6 mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/10 bg-white/5 backdrop-blur">
            ...
            <span className="text-sm font-medium text-white/80">My works &amp; projects</span>
          </div>
        </motion.div>
        ...
        >
          ...
          />
          <svg className="absolute w-4 h-4 -translate-y-1/2 right-5 top-1/2 text-white/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map((project, i) => (
              ...
              >
                {/* Image / Preview area */}
                <div className={`relative h-52 bg-gradient-to-br ${project.previewBg} overflow-hidden`}>
                  {/* Mock browser bar */}
                  <div className="absolute top-0 left-0 right-0 h-7 bg-black/40 backdrop-blur flex items-center px-3 gap-1.5 z-10">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pt-7">
                    <div className="mb-2 text-5xl font-bold text-white/20" style={{ fontFamily: 'var(--font-display)' }}>
                      {project.previewText.charAt(0)}
                    </div>
                    <p className="text-sm font-medium text-white/50">{project.previewText}</p>
                    <p className="mt-1 text-xs text-white/30">{project.previewSub}</p>
                  </div>
                  {/* Hover overlay with GitHub badge */}
                  {hoveredId === project.id && project.links.github && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="absolute flex items-center justify-center w-10 h-10 border rounded-full top-10 right-3 bg-white/10 backdrop-blur border-white/20"
                    >
                  ...
                  {hoveredId === project.id && !project.links.github && project.links.live && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="absolute px-3 py-1 font-mono text-xs border rounded-full top-10 right-3 bg-cyan-400/20 border-cyan-400/40 text-cyan-400"
                    >
                      Live
                    </motion.div>
                  )}
                ...
                <div className="flex flex-col flex-1 p-5">
                  <h3 className="mb-2 text-base font-bold text-white transition-colors duration-300 group-hover:text-cyan-300" style={{ fontFamily: 'var(--font-display)' }}>
                    {project.title}
                  </h3>
                  <p className="flex-1 mb-4 text-sm leading-relaxed text-white/50">{project.description}</p>

                  {/* Tags */}
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-3 py-1 font-mono text-xs transition-colors border rounded-full cursor-default border-white/15 text-white/50 hover:border-white/30">
                        {tag}
                      </span>
                    ))}
                  </div>

                  {/* Visit button — shows on hover like screenshot */}
